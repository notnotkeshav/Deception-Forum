...
<head>
   ...
   <title>⛧ Create New Thread - Red Skull ⛧</title>
...
<body>
   ...
   <div class="main-container">
      <h2 class="page-title">⛧ CREATE NEW THREAD ⛧</h2>

      <form id="createThread" method="POST" class="needs-validation" novalidate autocomplete="off">
         <div class="form-group">
            <label for="title" class="form-label">THREAD TITLE:</label>
            <input type="text" id="title" name="title" class="form-control" required autocomplete="off" spellcheck="false" autofocus>
         ...
         <div class="form-group">
            ...
            <input type="text" id="category" name="category" class="form-control" required autocomplete="off" spellcheck="false">
         ...
         <div id="imageUrlFields" class="form-group">
            ...
            <div class="input-group mb-2">
               <input type="text" name="imageUrl[]" class="form-control imageUrl" required autocomplete="off" spellcheck="false">
         ...
         <button type="submit" class="btn btn-primary btn-large">CREATE NOW!</button>
      </form>

      <div id="error-block" class="error-block">
         <?= isset($error['msg']) ? '<div class="alert alert-danger">' . htmlspecialchars($error['msg']) . '</div>' : ''; ?>
      </div>
   </div>
   ...
   <script>
      ...
      // Dynamic image fields (vanilla JS for best compatibility)
      ...
      // Remove button handler for initial field
      ...
      $(document).ready(() => {
         ...
         $('#createThread').on('submit', function(event) {
            ...
            sendRequest(
               ...
               (response) => {
                  if (response.success) {
                     showSuccess('Redirecting to threads...');
                     setTimeout(() => window.location.href = '/threads', 1200);
                  } else {
                     showError(response.error || 'Thread creation failed.');
                  }
               }
            );
         });
